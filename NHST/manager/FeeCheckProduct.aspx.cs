...
{
    public partial class FeeCheckProduct : System.Web.UI.Page
    {
        ...
        public void pagingall(List<tbl_FeeCheckProduct> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = FromRow; i < ToRow + 1; i++)
                {
                    ...
                    string type = "";
                    if (item.Type == 1)
                        type = "<span class=\"badge green darken-2 white-text border-radius-2\">Trên 10 tệ</span>";
                    else
                        type = "<span class=\"badge blue darken-2 white-text border-radius-2\">Dưới 10 tệ</span>";
                    ...
                    //hcm.Append("<a href=\"\" Onclick=\"Delete(" + item.ID + ")\" class=\"edit-mode tooltipped\" data-position=\"top\" ");
                    //hcm.Append(" data-tooltip=\"Xóa\"><i class=\"material-icons\">delete</i></a>");
                    hcm.Append("</div>");
                    hcm.Append("</td>");
                    hcm.Append("</tr>");
                }
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (fee != null)
            {
                ...
                string f = txtEditAmountFrom.Text.Replace(",", string.Empty);
                string t = txtEditAmountTo.Text.Replace(",", string.Empty);
                if (!string.IsNullOrEmpty(p))
                {
                    double price = Convert.ToDouble(p);
                    double AmountFrom = Convert.ToDouble(f);
                    double AmountTo = Convert.ToDouble(t);
                    var kq = FeeCheckProductController.Update(ID, AmountFrom, AmountTo, price, fee.Type.Value, DateTime.Now, ac.Username);
                    if (kq != null)
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (ac != null)
            {
                ...
                double price = Convert.ToDouble(p);
                var a = FeeCheckProductController.Insert(Convert.ToDouble(addAmountFrom.Text), Convert.ToDouble(addAmountTo.Text), price, 1, DateTime.Now, ac.Username);
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            if (Session["userLoginSystem"] != null)
            {
                int BankID = hdfIDFee.Value.ToInt(0);
                var kq = FeeCheckProductController.Delete(BankID);
                if (kq != null)
                {
